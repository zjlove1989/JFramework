...
TEST(EventBusTest, EventDataIntegrity)
{
	...
	// 验证事件数据完整性
	auto receivedEvent =
		std::dynamic_pointer_cast<ExtendedTestEvent>(handler.lastEvent);
...
TEST(BindablePropertyTest, MultipleObservers)
{
	...
	auto unregister1 =
		prop.Register([&](const std::string&) { notificationCount++; });
	auto unregister2 =
		prop.Register([&](const std::string&) { notificationCount++; });
...
TEST(BindablePropertyTest, MultipleRegistrations)
{
	...
	// 注册多个观察者
	auto unreg1 =
		prop.Register([&](int val) { notifications.push_back(val * 1); });
	auto unreg2 =
		prop.Register([&](int val) { notifications.push_back(val * 2); });
	auto unreg3 =
		prop.Register([&](int val) { notifications.push_back(val * 3); });
