...
// ========== ExceptionLogger 单元测试 ==========

class TestExceptionLogger : public IExceptionLogger
{
public:
	struct LogEntry
	{
	...
	{
		logs.push_back({ e.what(), context ? context : "" });
	}
};


TEST(ExceptionLoggerTest, LogExceptionIsCalledInEventBus)
{
	TestExceptionLogger logger;
	IExceptionLogger* oldLogger = ExceptionLoggerManager::GetLogger();
	ExceptionLoggerManager::SetLogger(&logger);

	EventBus bus;
	class ThrowingHandler : public ICanHandleEvent
	{
	public:
		void HandleEvent(std::shared_ptr<IEvent>) override
		{
			throw std::runtime_error("eventbus test error");
		}
	} handler;
	bus.RegisterEvent(typeid(TestEvent), &handler);
	EXPECT_NO_THROW(bus.SendEvent(std::make_shared<TestEvent>()));
	ASSERT_FALSE(logger.logs.empty());
	EXPECT_EQ(logger.logs[0].what, std::string("eventbus test error"));
	EXPECT_EQ(logger.logs[0].context, "EventBus::SendEvent");

	ExceptionLoggerManager::SetLogger(oldLogger);
}

TEST(ExceptionLoggerTest, LogExceptionIsCalledInBindableProperty)
{
	TestExceptionLogger logger;
	IExceptionLogger* oldLogger = ExceptionLoggerManager::GetLogger();
	ExceptionLoggerManager::SetLogger(&logger);
	...
	BindableProperty<int> prop(0);
	auto u1 = prop.Register([](const int&) { throw std::runtime_error("bindable test error"); });
	auto u2 = prop.Register([](const int&) { /* no throw */ });
	EXPECT_NO_THROW(prop.SetValue(42));
	ASSERT_FALSE(logger.logs.empty());
	EXPECT_EQ(logger.logs[0].what, std::string("bindable test error"));
	EXPECT_EQ(logger.logs[0].context, "BindableProperty::SetValue");

	ExceptionLoggerManager::SetLogger(oldLogger);
}

TEST(ExceptionLoggerTest, SetLoggerAndGetLogger)
{
	TestExceptionLogger logger;
	IExceptionLogger* oldLogger = ExceptionLoggerManager::GetLogger();
	ExceptionLoggerManager::SetLogger(&logger);
	EXPECT_EQ(ExceptionLoggerManager::GetLogger(), &logger);
	ExceptionLoggerManager::SetLogger(oldLogger);
	EXPECT_EQ(ExceptionLoggerManager::GetLogger(), oldLogger);
}
