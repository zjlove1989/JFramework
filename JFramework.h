...
namespace JFramework
{
	...
	// 具体注销实现
	template<typename T>
	class UnRegisterImpl : public AutoUnRegister
	{
	public:
		UnRegisterImpl(int id, BindableProperty<T>* property, std::function<void(T)> callback)
			: mProperty(property),
			mCallback(callback),
			mId(id)
		{
	...
	public:
		...
		// 注册观察者（带初始值通知）
		std::unique_ptr<AutoUnRegister> RegisterWithInitValue(std::function<void(const T&)> onValueChanged)
		{
		...
		// 注册观察者
		std::unique_ptr<AutoUnRegister> Register(std::function<void(const T&)> onValueChanged)
		{
			auto unRegister = std::make_unique<UnRegisterImpl<T>>(
		...
		void UnRegister(int id)
		{
			mObservers.erase(
				std::remove_if(mObservers.begin(), mObservers.end(),
					[id](const auto& item) {	return item->GetId() == id;	}), mObservers.end());
		}
	...
	private:
		...
		int mNextId = 0;
		T mValue;
		std::vector<UnRegisterImpl<T>*> mObservers;
	};
	...
	public:
		...
		std::shared_ptr<T> GetModel()
		{
			...
			if (!model)
			{
				throw std::runtime_error("Model not registered: " + std::string(typeid(T).name()));
			}
	...
	public:
		...
		std::shared_ptr<T> GetUtility()
		{
			...
			if (!utility)
			{
				throw std::runtime_error("Utility not registered: " + std::string(typeid(T).name()));
			}
	...
	public:
		...
		void RegisterEvent(ICanHandleEvent* handler)
		{
			...
			auto arch = GetArchitecture();
			if (!arch)
				throw  ArchitectureNotSetException(typeid(TEvent).name());
	...
	private:
		...
		// 显式特化声明
		template<>
		std::unordered_map<std::type_index, std::shared_ptr<IModel>>& GetContainer<IModel>() {
			return mModels;
		}

		template<>
		std::unordered_map<std::type_index, std::shared_ptr<ISystem>>& GetContainer<ISystem>() {
			return mSystems;
		}

		template<>
		std::unordered_map<std::type_index, std::shared_ptr<IUtility>>& GetContainer<IUtility>() {
			return mUtilitys;
		}
	...
	private:
		std::mutex m_mutex;
		std::unordered_map<std::type_index, std::vector<ICanHandleEvent*>> m_subscribers;
	};
	...
	public:
		...
		void RegisterSystem(std::type_index typeId, std::shared_ptr<ISystem> system) override
		{
			if (!system) {
		...
		void RegisterModel(std::type_index typeId, std::shared_ptr<IModel> model) override
		{
			if (!model) {
		...
		void RegisterUtility(std::type_index typeId, std::shared_ptr<IUtility> utility) override
		{
		...
		void RegisterEvent(std::type_index eventType, ICanHandleEvent* handler) override
		{
			mEventBus->RegisterEvent(eventType, handler);
		}

		void UnRegisterEvent(std::type_index eventType, ICanHandleEvent* handler) override
		{
